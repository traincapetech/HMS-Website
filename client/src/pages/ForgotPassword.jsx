...
import { FaRegEyeSlash, FaEye, FaEnvelope, FaLock, FaShieldAlt, FaKey } from "react-icons/fa";
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      if (result.payload && result.payload.success) {
        ...
        setShowEmail(false);
        setLoading(false);
        setSuccessMessage(result.payload.message || "OTP sent successfully. Please check your email.");
        
        // If we have a development OTP, pre-fill it (only in non-production)
        if (result.payload.devOtp) {
          const otpArray = result.payload.devOtp.split('');
          const newOtp = [...payload.otp];
        ...
        } else if (typeof result.payload === 'string' && result.payload.includes("not exist")) {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (result.payload && result.payload.success) {
        ...
        setSuccessMessage("Password changed successfully! You will be redirected to login in a moment.");
              ...
              <div className="bg-gradient-to-br from-blue-900 to-indigo-800 p-8 text-white md:w-2/5 flex flex-col justify-between">
                <div>
                  ...
                  <p className="opacity-80 mb-6">
                    Forgot your password? No problem. We'll help you reset it securely and get back to your account.
                  </p>
                </div>
                  ...
                  {showOtp && (
                    <div>
                      <p className="text-gray-600 mb-6">
                        Enter the 6-digit OTP sent to <span className="font-semibold">{payload.email}</span>
                      </p>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="relative">
                            ...
                            >
                              {passwordVisible ? <FaRegEyeSlash className="h-5 w-5 text-gray-400" /> : <FaEye className="h-5 w-5 text-gray-400" />}
                            </button>
                          </div>
