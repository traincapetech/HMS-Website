...
const createAppoint = async (req, res) => {
  try {
    ...
    console.log("THE BODY COMING HERE S ---->", req.body);//check the body
    ...
    <div class ="detail-item"><span class = "detail-label">Date: </span>${new Date(
      AppointDate
    ...
    await sendEmail({ to: doctorEmail, subject: Tamd, html: doctorEmailContent });
...
const countAppointments = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while counting appointments" });
  }
};
